...
export function getColorName(colorVal) {
    ...
    if (colorVal && Object.values(Colors).includes(colorVal)) {
        return U.objFindKey(Colors, (v) => v === colorVal);
    }
    return false;
}
export function getContrastingColor(colorVal, contrastLevel = 1, bgShade = "dark") {
    ...
    if (colorName && /GOLD|BLUE|RED/.test(colorName)) { // it's a color
        if (!/^[bgd]/.test(colorName)) { // it's a neutral color; refer to bgShade to nudge it brighter/darker
            if (bgShade === "light") {
                colorName = `d${masterColor}`;
            }
            else {
                colorName = `b${masterColor}`;
            }
        }
        // now any color value will be a bright/glow/dark shade
        if (/^[bg]/.test(colorName)) { // it's a bright shade
            return [false, Colors[`d${masterColor}`], Colors.BLACK, Colors.dBLACK, Colors.dBLACK][contrastLevel];
        }
        else { // it's a dark shade
            return [false, Colors[`b${masterColor}`], Colors[`g${masterColor}`], Colors.WHITE, Colors.bWHITE][contrastLevel];
        }
    }
    else { // it's a grey
