...
const sortIntoFolders = async (itemData: Partial<ItemDataRecord>, sortIntoSubFolders = false) => {
	const folderNames = {
	...
	if (sortIntoSubFolders) {
		...
		folders.forEach((fData) => {
			...
			folderMap[folderType as KeyOf<typeof folderMap>]["active-rolled"] = folderId;
			folderMap[folderType as KeyOf<typeof folderMap>]["active-static"] = folderId;
			folderMap[folderType as KeyOf<typeof folderMap>].passive = folderId;
		});
	}
