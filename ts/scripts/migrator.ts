...
const migrateData = (iData: ItemMigrationData): ItemMigrationData | false => {
	...
	const {record} = iData;
	if ("ammoCost" in record) { record.itemType = "attack" }
	...
	switch (record.itemType) {
		...
		case "move": {
			...
			if (record.type === "passive") {
				...
				let edges = stringToNum((record.results.success.text.match(/([^\s]+) Edge/) ?? [])[1] || 0),
								hold = stringToNum((record.results.success.text.match(/([^\s]+) Hold/) ?? [])[1] || 0);
...
export default function MIGRATE_ITEM_DATA(DATA: ItemMigrationData[]) {
