...
export default class kult4ePCsheet extends ActorSheet{
	get template(){
		return `systems/kult4th/templates/sheets/pc-sheet.hbs`;
	}
	...
	getData(){
		...
		data.moves = data.items.filter((item) => {return item.type == "move"} );
	...
	activateListeners(html) 	{
		...
		html.find(".stability-minus").click(ev =>{
			const stability_current = Number(this.actor.data.data.stability.value);
