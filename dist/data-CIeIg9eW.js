import { K as K4ItemType, a as K4ItemSubType, b as K4Attribute, c as K4ActiveEffect, A as AlertType, U as UserTargetRef, P as PromptInputType, d as K4ItemRange, E as EffectDuration, e as UserRef, f as K4ConditionType, g as EffectResetOn, h as U } from "./kult4th-bJeEWeuB.js";
...
const PACKS = {
  ...
  get [
    ...
    return this.subItems.filter(
      (move) => move.system.subType === "active-rolled"
      /* activeRolled */
    );
  },
  get [
    ...
    return this.subMoves.filter(
      (move) => move.system.subType === "active-static"
      /* activeStatic */
    );
  },
  get [
    ...
    return this.all.filter(
      (move) => move.system.subType === "passive"
      /* passive */
    );
  }
};
...
function getUniqueSystemKeys(itemDataArray, isExpanding = false) {
  const uniqueEntries = [];
  ...
  const parsedSubItemEntries = uniqueEntries.map((entry) => [entry[0], entry[1].join(", ")]);
...
function getUniqueValuesForSystemKey(itemDataArray, key) {
  ...
  itemDataArray.forEach((schema) => {
    ...
    if (key in flatSubItemSystem) {
      let thisValue = flatSubItemSystem[key];
...
function getItemSystemReport(itemDataArray = PACKS.all, options = {}) {
  ...
  let mapFunction;
  options.reportType ?? (options.reportType = "UniqueValues");
  switch (options.reportType) {
    case "UniqueValues":
      mapFunction = ([key, val]) => [
        key,
        VAL_TYPES_TO_LIST.includes(String(val)) ? getUniqueValuesForSystemKey(itemDataArray, key).join(", ") : val
      ];
      break;
    case "SubTypes":
      mapFunction = ([key]) => {
        const returnData = {
          [
            "active-rolled"
            /* activeRolled */
          ]: countSchemasWithSystemKey(
            itemDataArray.filter(
              (is) => is.system.subType === "active-rolled"
              /* activeRolled */
            ),
            key
          ),
          [
            ...
            itemDataArray.filter(
              (is) => is.system.subType === "active-static"
              /* activeStatic */
            ),
            key
          ),
  ...
  const reportObject = Object.fromEntries(
    Object.entries(keyTypeData).map(([key, val]) => mapFunction([key, val]))
  );
  return options.isExpanding !== false ? expandObject(reportObject) : reportObject;
}
function extractSubItemSchemas(itemDataArray = PACKS.all, subTypes = [
  "move"
  /* move */
]) {
  return itemDataArray.filter((item) => [
    "advantage",
    "disadvantage",
    "weapon",
    "gear"
    /* gear */
  ].includes(item.type)).map((parentItem) => parentItem.system.subItems.filter((subItem) => subTypes.includes(subItem.type))).flat();
}
function getUniqueSubItemSystemKeys(itemDataArray = PACKS.all, isExpanding = false) {
  return getUniqueSystemKeys(
    extractSubItemSchemas(itemDataArray),
    isExpanding
  );
}
function countSchemasWithSystemKey(schemaArray, key) {
  const schemasWith = [];
  const schemasWithout = [];
  const returnData = { count: 0, total: schemaArray.length };
...
function getUniqueValuesForSubItemKey(arg1, arg2) {
  let itemDataArray;
  let key;
  ...
  return getUniqueValuesForSystemKey(subItemDataArray, key);
}
function getSubItemSystemReport(itemDataArray = PACKS.all, options = {}) {
  const VAL_TYPES_TO_LIST = [
    "small-posInt",
    "boolean",
    "word-string",
  ...
  switch (options.reportType) {
    case "UniqueValues":
      mapFunction = ([key, val]) => [
        key,
        VAL_TYPES_TO_LIST.includes(String(val)) ? getUniqueValuesForSubItemKey(itemDataArray, key).join(", ") : val
      ];
      break;
    case "SubTypes":
      mapFunction = ([key, val]) => {
        const returnData = {
          [
            ...
            extractedSubItemSchemas.filter(
              (is) => is.system.subType === "active-rolled"
              /* activeRolled */
            ),
            key
...
function parseItemSchemasForCreation(itemDataArray = PACKS.all) {
  ...
  return itemDataArray.map((itemData) => {
    ...
    ["_id", "folder", "sort", "permission", "flags"].forEach((key) => {
      delete newItemData[key];
    });
    if (FOLDER_NAME_MAP[itemData.type]) {
      newItemData.folder = ((_b = (_a = getGame().folders) == null ? void 0 : _a.getName(FOLDER_NAME_MAP[itemData.type])) == null ? void 0 : _b.id) ?? null;
    }
    return newItemData;
  });
}
async function BUILD_ITEMS_FROM_DATA() {
  const itemSchemas = parseItemSchemasForCreation(PACKS.all);
  function clearActorItems(actor) {
  ...
  function clearAllActors() {
    return Promise.all(getGame().actors.map((actor) => clearActor(actor)));
  }
  await Promise.all([
    clearAllActors(),
    Promise.all(getGame().items.map((item) => item.delete()))
  ]);
  await Item.create(itemSchemas);
  await Promise.all(getGame().actors.map((actor) => actor.initMovesAndEffects()));
}
function findRepresentativeSubset(items) {
  function isNonEmpty(value) {
    return value !== "" && value !== void 0 && value !== null && !(Array.isArray(value) && value.length === 0) && value !== 0;
  }
  ...
  items.forEach((item) => {
    ...
    Object.entries(flatItem).forEach(([key, value]) => {
      if (isNonEmpty(value)) {
        if (!keyMap.has(key)) {
          keyMap.set(key, item);
          inclusionReasons[item.name] = key;
        }
      }
    });
  });
...
function checkSubsetCoverage(subset, masterSet) {
  function isNonEmpty(value) {
    return value !== "" && value !== void 0 && value !== null && !(Array.isArray(value) && value.length === 0) && value !== 0;
  }
...
function findUniqueKeys(subset, allItems) {
  function isNonEmpty(value) {
    return value !== "" && value !== void 0 && value !== null && !(Array.isArray(value) && value.length === 0) && value !== 0;
  }
...
//# sourceMappingURL=data-CIeIg9eW.js.map
