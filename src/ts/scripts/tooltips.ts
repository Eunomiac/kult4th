...
function OLD_handleTooltipTrigger(event: JQuery.MouseEnterEvent): void {
  ...
  if (tooltipId) {
    ...
    if (existingTooltip$.length) {
      ...
      if (existingAnimation) {
        ...
        // if (existingAnimation.timeline.isActive()) {
        //   if (existingAnimation.timeline.reversed()) {
        //     // Scenario 2: Tooltip is reversing, so we "unreverse" it
        //     existingAnimation.timeline.timeScale(1).play();
        //     updateTriggerOutline(trigger$, 'UNREVERSE');
        //   }
        //   // Scenario 3: An active, forward-playing tooltip is just fine: no change needed.
        // } else {
        //   existingAnimation
        // }
        // existingAnimation.timeline.pause();
