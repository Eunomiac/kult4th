...
class K4Actor extends Actor {
    ...
    promises.push(K4ActiveEffect.CreateFromBuildData(
      {
        parentData: K4ActiveEffect.BuildEffectData({
          ...
          tooltip: "<span class=\"block-text center\"><h2>Armor</h2></span><br/><br/>You are wearing armor that protects you from harm, conferring a bonus to your <span class=\"inline-doclink\" data-doc-name=\"Endure Injury\">Endure Injury</span> rolls."
        }),
        changeData: []
      }, this));
 ...
 async resetWoundName(id: IDString) {
  ...
  get statusBarEffects() {
    return this.effects.filter((effect) => effect.eData.inStatusBar && effect.isNonZero);
  }
 ...
 */
  preparePCData() {
    if (this.is(K4ActorType.pc)) {
      ...
      this.systemChanges.forEach((change) => { void change.apply(this) });

      this.system.statusBarEffects = this.statusBarEffects;
    }
  }
  /**
